...
class MainScreen(Screen):
    ...
    def add_bpm_data(self, bpm):
        if not self.measurement_active:
            return
        self.bpm_label.text = str(int(bpm))
        self.bpm_data.append(bpm)
        if len(self.bpm_data) > 50:
            self.bpm_data.pop(0)
        self.sample_count += 1
        ...
        if len(self.bpm_data) >= 40:
            middle_30 = self.bpm_data[5:35]  # 5개 앞, 5개 뒤 제외
            self.measurement_active = False
            self.manager.get_screen('loading').receive_data(middle_30)
            self.manager.current = 'loading'
...
class LoadingScreen(Screen):
    ...
    def receive_data(self, bpm_data):
        self.result_code = model_predict(bpm_data)
        print("Prediction result:", self.result_code)  # 출력 추가
...
class HeartApp(App):
    def build(self):
        ...
        # 센서 수신 스레드 시작
        threading.Thread(target=self.listen_to_sensor, daemon=True).start()

        return self.sm

    def listen_to_sensor(self):
        try:
            ser = serial.Serial('/dev/ttyACM0', 115200, timeout=1.1)
            time.sleep(2)
                ...
                if line.isdigit():
                    ...
                    def update(dt):
                        if self.sm.current == 'main':
                            self.sm.get_screen('main').add_bpm_data(bpm)
